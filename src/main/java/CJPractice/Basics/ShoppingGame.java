...
Then Aayush shops for 3 smartphones and then Harshit shops for 4 smartphones but that would
mean 2 + 4 = 6 smartphones in total. It's impossible because Harshit can shop for at most N smartphones,
so he loses. Aayush wins, and so we print "Aayush".
...
public class ShoppingGame {
    public static void main(String[] args) {
        ...
        while (no_test_cases > 0) {
            long M = s.nextLong(); //Aayush
            long N = s.nextLong(); //Harshit
            long ayush_phones_cnt = 0;
            long harshit_phones_cnt = 0;

            int phones_to_buy = 1;
            String winner = "Aayush";
            while (true) {
                if ((ayush_phones_cnt + phones_to_buy) <= M) {
                    winner = "Aayush";
                    ayush_phones_cnt += phones_to_buy;
                    phones_to_buy += 1;
                } else {
                    break;
                }
                if ((harshit_phones_cnt + phones_to_buy) <= N) {
                    winner = "Harshit";
                    harshit_phones_cnt += phones_to_buy;
                    phones_to_buy += 1;
