...
public class ClimbStairs_1_2_Steps_Order_Does_Not_Matter {
    static int ans1 = 0;
    ...
    public static int climbStairsDistinctPaths(int current_step,int step_taken, int dest_step, String path) {
        ...
        int sp1 = 0;
        if(step_taken <= 1){
            sp1 = climbStairsDistinctPaths(current_step + 1,1, dest_step, path + "1");
        }

        //Take 2 steps
        int sp2 = 0;
        if(step_taken <= 2){
            sp2 = climbStairsDistinctPaths(current_step + 2,2,  dest_step, path + "2");
        }
        int ans = sp1 + sp2;
